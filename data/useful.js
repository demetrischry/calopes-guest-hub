const usefulData = {

    /* =================================
       FOOD & DAILY ESSENTIALS
    ================================= */

    essentials: [

        {
            name: "Froutaria Theofanis",

            category: "essentials",

            badge: "Closest to Calopes",

            location: "Tsada",

            subtitle: "Fresh Produce & Local Essentials",

            typeIcon: "apple",

            distance: "10 min",

            description:
                "A convenient local fruit market in nearby Tsada, ideal for fresh fruit, vegetables, local produce and everyday groceries without travelling into Paphos.",

            image:
                "images/useful/theofanis.jpg",

            mapsLink:
                "https://www.google.com/maps/search/?api=1&query=Froutaria+Theofanis+Tsada+Cyprus",

            phone: "",

            website: ""
        },

        {
            name: "Express Supermarket • Fruitmarket",

            category: "essentials",

            badge: "Local Favourite",

            location: "Mesogi",

            subtitle: "Mini Market & Fruit Market",

            typeIcon: "shopping-basket",

            distance: "15 min",

            description:
                "A convenient neighbourhood supermarket in Mesogi offering fresh fruit and vegetables, groceries, drinks and everyday essentials. A useful stop on your way to or from Calopes Suites.",

            image:
                "images/useful/express.jpg",

            mapsLink:
                "https://www.google.com/maps/place/Express+Supermarket+%E2%80%A2+Fruitmarket/@34.8151352,32.4523172,18.22z/data=!4m6!3m5!1s0x14e70f53efdac009:0x638bea388b3e7ce!8m2!3d34.8156107!4d32.4513927!16s%2Fg%2F11c0x5qzxs?entry=ttu&g_ep=EgoyMDI2MDgwMi4wIKXMDSoASAFQAw%3D%3D",

            phone: "",

            website: ""
        },

        {
            name: "Lidl Paphos",

            category: "essentials",

            badge: "Best Value",

            location: "Paphos",

            subtitle: "Groceries & Everyday Essentials",

            typeIcon: "shopping-cart",

            distance: "20 min",

            description:
                "A familiar and affordable supermarket offering groceries, fresh bakery products, household supplies and everyday essentials. A practical choice for quick or budget-friendly shopping.",

            image:
                "images/useful/lidl.jpg",

            mapsLink:
                "https://www.google.com/maps/place/Lidl/@34.7930574,32.436439,15.78z/data=!4m6!3m5!1s0x14e7066ded12548b:0x7ce5da7d76a5ffae!8m2!3d34.79074!4d32.43418!16s%2Fg%2F11cm_gzrxp?entry=ttu&g_ep=EgoyMDI2MDgwMy4wIKXMDSoASAFQAw%3D%3D",
            phone: "",

            website: ""
        },

        {
            name: "Sklavenitis Supermarket",

            category: "essentials",

            badge: "Full Grocery Shop",

            location: "Hellados Avenue, Paphos",

            subtitle: "Large Supermarket",

            typeIcon: "shopping-cart",

            distance: "20 min",

            description:
                "A large supermarket with fresh produce, meat, bakery products, ready meals, household items and everything needed for a complete grocery shop during your stay.",

            image:
                "images/useful/sklavenitis.jpg",

            mapsLink:
                "https://www.google.com/maps/place/%CE%A3%CE%9A%CE%9B%CE%91%CE%92%CE%95%CE%9D%CE%99%CE%A4%CE%97%CE%A3+%CE%A0%CE%AC%CF%86%CE%BF%CF%82/@34.7884663,32.431533,17z/data=!3m1!4b1!4m6!3m5!1s0x14e7066e397a9319:0xff1cb94751ad1a9a!8m2!3d34.7884663!4d32.4341079!16s%2Fg%2F1tjjrl_x?entry=ttu&g_ep=EgoyMDI2MDgwMy4wIKXMDSoASAFQAw%3D%3D",

            phone:
                "+35726936600",

            website:
                "https://sklavenitiscyprus.com.cy/placemarks/sklavenitis-pafos-2/"
        },

        {
            name: "Papantoniou Bakery",

            category: "essentials",

            badge: "Fresh Every Day",

            location: "Mesogi",

            subtitle: "Bakery, Coffee & Takeaway Food",

            typeIcon: "croissant",

            distance: "15 min",

            description:
                "A convenient bakery for fresh bread, pastries, cakes, sandwiches, coffee and takeaway food. Ideal for breakfast, a quick snack or something easy to enjoy at the suites.",

            image:
                "images/useful/papantoniou.jpg",

            mapsLink:
                "https://www.google.com/maps/place/%CE%A0%CE%B1%CF%80%CE%B1%CE%BD%CF%84%CF%89%CE%BD%CE%AF%CE%BF%CF%85+%CE%91%CF%81%CF%84%CE%BF%CE%B6%CE%B1%CF%87%CE%B1%CF%81%CE%BF%CF%80%CE%BB%CE%B1%CF%83%CF%84%CE%B5%CE%AF%CE%BF/@34.8161189,32.451718,19.37z/data=!4m6!3m5!1s0x14e70f00544be069:0x71580d5278e3bd04!8m2!3d34.8164976!4d32.4513518!16s%2Fg%2F11y86t2jcl?entry=ttu&g_ep=EgoyMDI2MDgwMi4wIKXMDSoASAFQAw%3D%3D",

            phone: "",

            website:
                "https://www.papantonioubakeries.com/"
        }

    ],


    /* =================================
       PHARMACIES & MEDICAL
    ================================= */

    medical: [

        {
            name: "Maria Nikolaidou Pharmacy",

            category: "medical",

            badge: "Closest Pharmacy",

            location: "Mesogi",

            subtitle: "Pharmacy & Health Products",

            typeIcon: "pill",

            distance: "15 min",

            description:
                "The closest pharmacy to Calopes Suites, located in nearby Mesogi. Ideal for prescriptions, healthcare products and everyday essentials. Opening hours may vary, so please check availability before visiting.",

            image:
                "images/useful/maria-nikolaidou-pharmacy.jpg",

            mapsLink:
                "https://www.google.com/maps/place/%CE%A6%CE%B1%CF%81%CE%BC%CE%B1%CE%BA%CE%B5%CE%AF%CE%BF+%CE%9C%CE%B1%CF%81%CE%AF%CE%B1+%CE%9D%CE%B9%CE%BA%CE%BF%CE%BB%CE%B1%CE%AF%CE%B4%CE%BF%CF%85/@34.8150891,32.4519054,19.24z/data=!4m6!3m5!1s0x14e70f8e9137f22b:0xdf85c49ab78dc394!8m2!3d34.8154538!4d32.4518004!16s%2Fg%2F11fsr8rkln?entry=ttu&g_ep=EgoyMDI2MDgwMi4wIKXMDSoASAFQAw%3D%3D",

            phone: "",

            website: ""
        },

        {
            name: "Paphos General Hospital",

            category: "medical",

            badge: "Hospital",

            location: "Anavargos, Paphos",

            subtitle: "Emergency & Medical Care",

            typeIcon: "hospital",

            distance: "20 min",

            description:
                "The main public hospital serving the Paphos district, providing emergency treatment and a wide range of medical services. For a serious or life-threatening emergency, call 112.",

            image:
                "images/useful/hospital.jpg",

            mapsLink:
                "https://www.google.com/maps/search/?api=1&query=Paphos+General+Hospital+Anavargos+Cyprus",

            phone:
                "+35726803100",

            website:
                "https://www.shso.org.cy/en/pos-na-mas-vreite-geniko-nosokomeio-pafou/"
        }

    ],


    /* =================================
       EMERGENCY CONTACTS
    ================================= */

    emergency: [

        {
            title: "European Emergency Number",

            subtitle:
                "Police, ambulance or fire service",

            number: "112",

            callLink: "tel:112",

            icon: "siren"
        },

        {
            title: "Cyprus Emergency Services",

            subtitle:
                "Alternative emergency number",

            number: "199",

            callLink: "tel:199",

            icon: "shield-alert"
        }

    ]

};