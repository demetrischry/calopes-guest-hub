const placesData = [

    /* =================================
       DISCOVER KALLEPEIA
    ================================= */

    {
        name: "Kallepeia Village Centre",

        category: "kallepeia",

        badge: "Village Life",

        location: "Kallepeia",

        subtitle: "Traditional Cypriot Village",

        typeIcon: "house",

        distance: "2 min",

        description:
            "Discover the authentic character of Kallepeia through its peaceful streets, traditional stone houses and relaxed village atmosphere. A lovely place for a quiet walk close to Calopes Suites.",

        image:
            "images/places/kallepeia.jpg",

        mapsLink:
            "https://www.google.com/maps/dir//%CE%9A%CE%B1%CE%BB%CE%BB%CE%AD%CF%80%CE%B5%CE%B9%CE%B1,+%CE%A0%CE%AC%CF%86%CE%BF%CF%82+8541,+%CE%9A%CF%8D%CF%80%CF%81%CE%BF%CF%82/@34.8397806,32.4712029,13z/data=!4m19!1m9!3m8!1s0x14e70e5a66a9ade1:0x6902de346f48e2aa!2zzprOsc67zrvOrc-AzrXOuc6xLCDOoM6sz4bOv8-CIDg1NDEsIM6az43PgM-Bzr_Pgg!3b1!8m2!3d34.8461805!4d32.5006167!15sCh9LYWxsZXBlaWEgVmlsbGFnZSBDZW50cmUgQ3lwcnVzkgEIbG9jYWxpdHngAQA!16s%2Fm%2F065_lj6!4m8!1m0!1m5!1m1!1s0x14e70e5a66a9ade1:0x6902de346f48e2aa!2m2!1d32.5006167!2d34.8461805!3e3?entry=ttu&g_ep=EgoyMDI2MDgwMy4wIKXMDSoASAFQAw%3D%3D",
        phone: "",

        website: ""
    },

    {
        name: "Agios Georgios Church",

        category: "kallepeia",

        badge: "Historic Church",

        location: "Kallepeia",

        subtitle: "Local Religious Landmark",

        typeIcon: "church",

        distance: "2 min",

        description:
            "A historic church in the heart of Kallepeia, closely connected to the life and traditions of the local community. Worth visiting while exploring the village centre.",

        image:
            "images/places/agiosgeorgios.jpg",

        mapsLink:
            "https://www.google.com/maps/place/%CE%99%CE%B5%CF%81%CF%8C%CF%82+%CE%9D%CE%B1%CF%8C%CF%82+%CE%91%CE%B3%CE%AF%CE%BF%CF%85+%CE%93%CE%B5%CF%89%CF%81%CE%B3%CE%AF%CE%BF%CF%85,+%CE%9A%CE%B1%CE%BB%CE%BB%CE%AD%CF%80%CE%B5%CE%B9%CE%B1/@34.8458983,32.5000951,17.64z/data=!4m23!1m16!4m15!1m6!1m2!1s0x14e70f616f343787:0xa2e7db2c84f25265!2zzpnOtc-Bz4zPgiDOnc6xz4zPgiDOkc6zzq_Ov8-FIM6TzrXPic-BzrPOr86_z4UsIM6azrHOu867zq3PgM61zrnOsSwgUkdXMitGNkgsIM6azrHOu867zq3PgM61zrnOsSwgzqDOrM-Gzr_PgiA4NTQxLCDOms-Nz4DPgc6_z4I!2m2!1d32.5009777!2d34.8461407!1m6!1m2!1s0x14e70f616f343787:0xa2e7db2c84f25265!2zzpnOtc-Bz4zPgiDOnc6xz4zPgiDOkc6zzq_Ov8-FIM6TzrXPic-BzrPOr86_z4UsIM6azrHOu867zq3PgM61zrnOsSwgUkdXMitGNkgsIM6azrHOu867zq3PgM61zrnOsSwgzqDOrM-Gzr_PgiA4NTQxLCDOms-Nz4DPgc6_z4I!2m2!1d32.5009777!2d34.8461407!3e3!3m5!1s0x14e70f616f343787:0xa2e7db2c84f25265!8m2!3d34.8461407!4d32.5009777!16s%2Fg%2F11l1n8sjx0?entry=ttu&g_ep=EgoyMDI2MDgwMy4wIKXMDSoASAFQAw%3D%3D",

        phone: "",

        website: ""
    },

    {
        name: "Monastery of the Holy Cross",

        category: "kallepeia",

        badge: "Historic Monastery",

        location: "Kallepeia",

        subtitle: "Peaceful Religious Site",

        typeIcon: "church",

        distance: "5 min",

        description:
            "A peaceful religious site surrounded by the countryside of Kallepeia. Its quiet setting and local significance make it a meaningful stop during your visit to the village.",

        image:
            "images/places/monastiri.jpg",

        mapsLink:
            "https://www.google.com/maps/place/%CE%91%CF%81%CF%87%CE%B1%CE%AF%CE%BF+%CE%9C%CE%BD%CE%B7%CE%BC%CE%B5%CE%AF%CE%BF+%CE%A4%CE%B9%CE%BC%CE%AF%CE%BF%CF%85+%CE%A3%CF%84%CE%B1%CF%85%CF%81%CE%BF%CF%8D+%CE%9C%CE%BF%CE%BD%CE%B1%CF%83%CF%84%CE%B7%CF%81%CE%BF%CF%8D%CE%B9+(%CE%B5%CF%81%CE%B5%CE%AF%CF%80%CE%B9%CE%B1)/@34.8463437,32.5000076,18z/data=!4m9!1m2!2m1!1sMonastery+of+the+Holy+Cross+Kallepeia+Cyprus!3m5!1s0x14e70f0033d1590d:0x44f2eaee2835c439!8m2!3d34.8463438!4d32.501841!16s%2Fg%2F11wv7ybf5y?entry=ttu&g_ep=EgoyMDI2MDgwMy4wIKXMDSoASAFQAw%3D%3D",

        phone: "",

        website: ""
    },


    /* =================================
       HISTORIC & CULTURAL
    ================================= */

    {
        name: "Minthis Monastery",

        category: "historic",

        badge: "Historic Monastery",

        location: "Tsada",

        subtitle: "Religious Heritage",

        typeIcon: "church",

        distance: "10 min",

        description:
            "A historic monastery set within the peaceful countryside near Tsada. Its traditional architecture and quiet surroundings offer a calm cultural stop close to Calopes Suites.",

        image:
            "images/places/minthis.jpg",

        mapsLink:
            "https://www.google.com/maps/search/?api=1&query=Minthis+Monastery+Tsada+Cyprus",

        phone: "",

        website: ""
    },

    {
        name: "Saint Neophytos Monastery",

        category: "historic",

        badge: "Byzantine Heritage",

        location: "Tala",

        subtitle: "Monastery & Rock-Cut Hermitage",

        typeIcon: "church",

        distance: "15 min",

        description:
            "A peaceful historic monastery famous for its rock-carved hermitage, religious artwork and beautiful natural surroundings. One of the most important cultural sites near Paphos.",

        image:
            "images/places/agiosneofutos.jpg",

        mapsLink:
            "https://www.google.com/maps/search/?api=1&query=Saint+Neophytos+Monastery+Paphos+Cyprus",

        phone: "",

        website: ""
    },

    {
        name: "Tombs of the Kings",

        category: "historic",

        badge: "UNESCO Site",

        location: "Paphos",

        subtitle: "Ancient Archaeological Landmark",

        typeIcon: "landmark",

        distance: "25 min",

        description:
            "An impressive ancient burial complex carved into solid rock, offering a fascinating look into the history of Paphos. Its underground chambers and open courtyards make it one of the area's most memorable landmarks.",

        image:
            "images/places/tafoivasileon.jpg",

        mapsLink:
            "https://www.google.com/maps/search/?api=1&query=Tombs+of+the+Kings+Paphos+Cyprus",

        phone: "",

        website: ""
    },

    {
        name: "Paphos Archaeological Park",

        category: "historic",

        badge: "Ancient Paphos",

        location: "Kato Paphos",

        subtitle: "Roman Mosaics & Ruins",

        typeIcon: "landmark",

        distance: "25 min",

        description:
            "One of Cyprus' most important archaeological sites, known for its Roman villas, detailed mosaics and ancient ruins. A must-visit for anyone interested in the history of Paphos.",

        image:
            "images/places/paphos-archaeological-park.jpg",

        mapsLink:
            "https://www.google.com/maps/search/?api=1&query=Paphos+Archaeological+Park+Cyprus",

        phone: "",

        website: ""
    },

    {
        name: "Paphos Old Town",

        category: "historic",

        badge: "Old Town",

        location: "Paphos",

        subtitle: "Markets, Streets & Local Culture",

        typeIcon: "building-2",

        distance: "25 min",

        description:
            "Explore the character of Paphos through its traditional streets, local market, cafés and restored public spaces. A pleasant area for walking, shopping and discovering everyday local life.",

        image:
            "images/places/paphos-old-town.jpg",

        mapsLink:
            "https://www.google.com/maps/search/?api=1&query=Paphos+Old+Town+Cyprus",

        phone: "",

        website: ""
    },


    /* =================================
       NATURE & VIEWS
    ================================= */

    {
        name: "Edro III Shipwreck",

        category: "nature",

        badge: "Shipwreck",

        location: "Peyia",

        subtitle: "Coastal Landmark",

        typeIcon: "ship",

        distance: "35 min",

        description:
            "A striking shipwreck resting close to the rocky coastline, creating one of the most distinctive views in Paphos. Especially impressive during the late afternoon and sunset.",

        image:
            "images/places/edro-iii-shipwreck.jpg",

        mapsLink:
            "https://www.google.com/maps/search/?api=1&query=Edro+III+Shipwreck+Peyia+Cyprus",

        phone: "",

        website: ""
    },

    {
        name: "Adonis Baths",

        category: "nature",

        badge: "Waterfalls",

        location: "Near Koili",

        subtitle: "Natural Pools & Waterfalls",

        typeIcon: "droplets",

        distance: "25 min",

        description:
            "A natural attraction with waterfalls, rock pools and a peaceful countryside setting. A memorable stop for guests looking for a different experience away from the coast.",

        image:
            "images/places/adonis-baths.jpg",

        mapsLink:
            "https://www.google.com/maps/search/?api=1&query=Adonis+Baths+Paphos+Cyprus",

        phone: "",

        website: ""
    },


];